...
def fishing_loop():
    ...
    # if we get nothing for last result then check if we should repair or move
    if(gv.last_results.is_full_of('0')):

        # check if the character should move left right forward and backward
        if(dict['moving']['enable'].get() == 1):
            debug("Moving is enabled")
            should_move_in = -1 * (int(time()) - gv.last_move_time - dict['moving']['every'].get())
            debug("Moving in:" + str(should_move_in))
            if (should_move_in < 0):
                gv.last_move_time = int(time())
                info("Moving")
                move_left_right()

        # check if we should be repairing and adding bait onto the line - do so if we should
